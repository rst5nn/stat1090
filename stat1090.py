...
try:
    ...
    class DummyCollectd:
        ...
        def register_read(self, func, interval=60, **kwargs): pass
        def register_config(self, callback, name=None): pass
        ...
        if child.key == 'Instance':
            ...
            if url:
                collectd.register_read(callback=read_1090,
                                       ...
                                       name='stat1090.' + instance_name,
                                       interval=60)
                collectd.register_read(callback=read_traffic,
                                       name='stat1090_traffic.' + instance_name,
                                       interval=60)
...
collectd.register_config(callback=handle_config, name='stat1090')


# =====================================================================
# stat1090 Landing & Departure Traffic Spotter Module
# =====================================================================
ACTIVE_LANDINGS = {}    # { hex: timestamp }
ACTIVE_DEPARTURES = {}  # { hex: timestamp }
CONFIG_PATH = "/etc/default/stat1090.conf"
...
def load_traffic_config():
    ...
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            collectd.warning("stat1090 plugin: error parsing config %s: %s" % (CONFIG_PATH, str(e)))
    return config
...
def read_traffic(data=None):
    ...
    if not os.path.exists(data_path):
        data_path = "/usr/share/stat1090/data-symlink/data/aircraft.json"
    ...
    if os.path.exists(data_path):
        try:
            ...
            collectd.warning("stat1090 traffic plugin: error reading %s: %s" % (data_path, str(e)))
...
# =====================================================================
# stat1090 Thermal Zone Temperature Collector Module
# =====================================================================
